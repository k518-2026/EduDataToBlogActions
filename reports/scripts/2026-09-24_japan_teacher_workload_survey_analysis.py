"""
【教員勤務実態調査】小・中学校教員の在校等時間と授業準備・持ち帰り仕事の推移
オープンデータ統計分析・可視化再現スクリプト
データ提供元: 文部科学省「教員勤務実態調査」 (https://www.mext.go.jp/a_menu/shotou/gyouseki/detail/1416756.htm)
"""
import pandas as pd
import numpy as np
...
# ==============================================================================
metrics = ['在校等時間', '授業準備時間', '部活動指導時間', '持ち帰り仕事時間']
print("\n==================================================")
print("📊 主要指標の記述統計量")
print("==================================================")
for m in metrics:
    if m in df.columns and pd.api.types.is_numeric_dtype(df[m]):
        ...
        print(f"  平均値: {s.mean():.2f} 時間")
        print(f"  中央値: {s.median():.2f} 時間")
    ...
    for m in metrics:
        if m in df_sorted.columns:
            ...
            if len(sub) >= 2:
                ...
                print(f"  変化量: {diff:+.2f} 時間 (変化率: {pct:+.1f}%)")
...
print("==================================================")
nc_pairs = [('在校等時間', '持ち帰り仕事時間'), ('授業準備時間', '部活動指導時間'), ('部活動指導時間', '持ち帰り仕事時間'), ('授業準備時間', '持ち帰り仕事時間'), ('在校等時間', '授業準備時間'), ('在校等時間', '部活動指導時間')]
...
time_col = "調査年"
metric = "在校等時間"
for grp in df["校種"].unique():
    sub = df[df["校種"] == grp].sort_values(by=time_col)
...
plt.ylabel(f"{metric} (時間)", fontsize=11)
plt.legend(title="【帯・誤差棒: 95% CI】", frameon=True, facecolor="white")

plt.title("【教員勤務実態調査】小・中学校教員の在校等時間と授業準備・持ち帰り仕事の推移", fontsize=13, fontweight="bold", pad=12)
