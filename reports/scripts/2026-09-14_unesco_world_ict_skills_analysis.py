"""
【UNESCO / ITU】若年層・成人のプログラミング・デジタルスキル保有率の国際比較 (SDG 4.4.1)
オープンデータ統計分析・可視化再現スクリプト
データ提供元: UNESCO Institute for Statistics (UIS) / ITU Database (http://data.uis.unesco.org/)
"""
import pandas as pd
import numpy as np
...
# グループ別（国・地域等）の平均値横棒グラフ
metric = "プログラミングスキル保有率"
ranked = df.groupby("国名")[metric].mean().sort_values(ascending=True)
bars = plt.barh(ranked.index, ranked.values, color="#457b9d", height=0.65)
for bar in bars:
    w = bar.get_width()
    plt.text(w + 0.3, bar.get_y() + bar.get_height() / 2, f"{w:.1f}%", va="center", fontweight="bold", fontsize=9)
plt.xlabel(f"{metric} (%)", fontsize=11)

plt.title("【UNESCO / ITU】若年層・成人のプログラミング・デジタルスキル保有率の国際比較 (SDG 4.4.1)", fontsize=13, fontweight="bold", pad=12)
