...
{{
  ...
  "scores": {{
    ...
    "信頼性・統計的妥当性": ["C+", "マクロ集計値の分析に基づきながら児童生徒個人の認知的変容を論じる点に生態学的誤謬（Ecological Fallacy）の懸念があり、未統制の交絡因子（SES等）や相関と因果の峻別についての論述が不足している。"],
...
"""
    ...
    def _generate_template_fallback(
        ...
        scores = {
            ...
            "信頼性・統計的妥当性": (
                "C+",
                "公的集計値（マクロデータ）を用いているため、集計単位の相関から個々の児童生徒の認知的メカニズムを推論する際の"
                "「生態学的誤謬（Ecological Fallacy）」のリスクが懸念される．家庭のSES等の交絡因子が未統制である限界の明記が不可欠である．",
            ),
            "論理的一貫性・構成": (
                "A-",
