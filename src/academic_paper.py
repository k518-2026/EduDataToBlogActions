...
日本教育工学会論文誌（JSET）の投稿規程および執筆の手引（教育実践研究論文／資料）に厳格に準拠した、極めて学術性の高い本格的な学術論文を執筆してください。
...
2. **各フィールドの記述要件**:
   ...
   - **abstract**: 和文抄録。320〜380文字（JSET規定：400字以内かつ8割以上を満たすこと）。
   ...
   - **references**: 5〜7件の実在する信頼できる学術文献リスト。著者の苗字アルファベット順。JSET形式（著者名 (年) 題名. 雑誌名, <b>巻</b> (号) ：ページ番号.）。
   - **title_en**: 英語論文タイトル。
