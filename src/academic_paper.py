...
2. **各フィールドの記述要件**:
   ...
   - **background**: 800〜1100文字。問題の社会的・教育的背景を論理的かつ丁寧に詳述すること。★【必須】海外の学術論文・国際報告書（OECD PISA, IEA TIMSS, またはUNESCO, Wing等の海外著名文献）を必ず2本以上直接引用（著者名・年号）し、国際的動向から国内のカリキュラム改革（学習指導要領・GIGAスクール構想）の現状と課題、実証的データ分析（EBPM）の不可欠性へと論理的・丁寧に接続すること。
   ...
   - **results_text**: 800〜1100文字。★【必須】必ず【RQ1に関する結果】→【RQ2に関する結果】の順で記述すること。「表１」（記述統計・分布特性）、「表２」（回帰分析）、「図１」（推移トレンド）、「図２」（相関・格差）の図表を参照しながら実測数値を網羅して客観的に記述すること。
   - **discussion**: 1000〜1400文字。★【必須】必ず【RQ1に関する考察】→【RQ2に関する考察】の順で記述すること。
     - RQ1に関する考察では、先行研究を2本引用し、「本結果と同じところ（共通点）」と「本結果と違うところ（相違点）」を対比して教育学的考察を行うこと。
     - RQ2に関する考察では、別の先行研究を2本引用し、「本結果と同じところ（共通点）」と「本結果と違うところ（相違点）」を対比して教育学的考察を行うこと。
     - 節の末尾に必ず「今後の課題（研究の限界および今後の展望）」を明記すること。
   - **references**: ★【必須】厳密に4件の実在する信頼できる学術文献リスト（海外文献2件＋国内文献2件、計4件）。著者の苗字アルファベット順。標準的な学術論文誌形式（著者名 (年) 題名. 雑誌名, <b>巻</b> (号) ：ページ番号.）。背景で引用した海外文献2件、および考察で引用した先行研究（RQ1で2件、RQ2で2件、計4件）と完全に一致させること。特定の学会名（「日本教育工学会」等）は含めないこと。
   - **title_en**: 英語論文タイトル。
    ...
    def _generate_template_fallback(
        ...
        if is_math:
            ...
            background = (
                ...
                "我が国においても，文部科学省の新学習指導要領において「数学的な見方・考え方」を働かせた探究的・協働的な問題解決能力の育成が中核に位置づけられ，"
                "主体的・対話的で深い学びの実現に向けた授業改善が推進されている．しかしながら，学校現場の実態としては，学年の進行に伴う数学への苦手意識の固定化や，"
                "地域間・学校種別間における学力格差の顕在化など，克服すべき課題が依然として山積している．"
            ...
            results_text = (
                ...
                " 表２に示す通り，時系列回帰モデルの推定により回帰勾配および決定係数が算出され，図１の推移チャートからも経年的な変化の方向性が視覚的に裏付けられた．\n"
                f"さらに，指標間の関連性分析（図２参照）においては，{corr_desc if corr_desc else '各指標間において特有の連動性が確認された．'}"
                " 図２の散布図・属性比較グラフが示す通り，学力水準と学習肯定感との間には統計的に有意な構造的連関性が明瞭に表出している．"
            )
            discussion = (
                ...
                "一方で，OECD (2023) のPISA 2022国際比較では高学力層と低学力層の乖離拡大が国際的な傾向として警鐘を鳴らされているが，本データセットのIQR（表１）は依然として狭小な範囲にとどまっており，"
                "初等中等教育段階における基礎的リテラシーの均質性が一定程度保たれている点は国際的傾向と異なる本邦特有の知見（違うところ）である．\n\n"
                ...
                "しかしながら，Mullis et al. (2020) のTIMSS報告では好意度の向上と正答率の向上が同時並行で進展すると示唆されているのに対し，"
                "本研究の時系列回帰分析（表２・図１）では，学習好意度の微増にもかかわらず特定学年において緩やかな下降トレンドが検出された．"
                "この乖離（違うところ）は，単なる情意の改善にとどまらず，授業内での深い概念的理解への転換が不可欠であることを示唆している．\n\n"
                "【研究の限界と今後の課題】\n"
            ...
            background = (
                ...
                "我が国においても，文部科学省の主導によるGIGAスクール構想の下で，児童生徒1人1台端末および高速大容量通信環境の配備が前倒しで達成された．"
                "小学校におけるプログラミング教育の必修化，中学校技術分野の拡充，さらには高等学校における共通必履修科目「情報I」の新設と大学入学共通テストへの導入など，"
                "小・中・高を通じた体系的な情報教育の制度設計が相次いで施行されている．しかしながら，日常的な授業での活用頻度，児童生徒の情報活用能力の育成度合い，"
                "ならびに自治体間・学校種別間の指導格差など，運用段階における課題が深刻化している．"
            ...
            results_text = (
                ...
                " 表２に示すトレンド指標および図１の推移グラフからも，近年の急速な進展と今後の定着に向けた課題が視覚化された．\n"
                f"さらに，指標間の相関分析（図２参照）においては，{corr_desc if corr_desc else '各指標間において構造的な連関性が検出された．'}"
                " 図２の散布図・属性比較グラフが明瞭に示す通り，ハードウェアの整備進捗と日常的な探究活用・学習成果との間には有意な正の相関構造が存在している．"
            )
            discussion = (
